...
export class BancPage {
  ...
  getDoneActivities() {
    ...
    this.activityRequestService.getMyPetitions(id).subscribe(data => {
      const da: any = data;
    ...
    this.activityRequestService.getTheirPetitions(id).subscribe(data => {
      const da2: any = data;
  ...
  getWallet() {
    const id = localStorage.getItem('userId');
    this.userService.getUserWallet(id).subscribe(data => {
      this.user = data;
      console.log(data);
    }, data => {
      console.error(data);
